# -*- coding: utf-8 -*-
"""
Created on Tue Apr 04 17:01:46 2017
...
def corAfterPer(p,rad,imgSize1,imgSize2):#旋转后的新坐标
    p1 = corTransToCenter(p,imgSize1)
    p2 = corPerspectiveTrans(p1,rad)
    p3 = corTransRestore(p2,imgSize2)
    return p3

def corAfterInvPer(p,rad,imgSize1,imgSize2):#逆旋转后的新坐标
    p1 = corTransToCenter(p,imgSize1)
    p2 = corInvPerspectiveTrans(p1,rad)
    p3 = corTransRestore(p2,imgSize2)
    return p3

def getNewSize(rad,size):#旋转后新图像的大小
    p1 = corPerspectiveTrans(corTransToCenter((0,0),size),rad)
    p2 = corPerspectiveTrans(corTransToCenter((0,size[1]),size),rad)
    p3 = corPerspectiveTrans(corTransToCenter((size[0],0),size),rad)
    p4 = corPerspectiveTrans(corTransToCenter((size[0],size[1]),size),rad)
...
def getPerspectivedImg(img,angle):#双线性插值，角落部分用原图片的边界所有像素均值进行填充
    ...
    for i in fourSidesCor:
        fourSidesCorAfterRot[index,:] = corAfterPer(i,rad,(height,width),(newHeight,newWidth))
        index = index + 1
    ...
    for i in range(0,newHeight):
        for j in range(imageRegion[i,0],imageRegion[i,1] + 1):
            p = corAfterInvPer((i,j),rad,(newHeight,newWidth),(height,width))
...
def main():
    ...
    newImg= getPerspectivedImg(img,angle)
